...
static struct error_desc {
  ...
  const char *message;
    ...
    // clang-format off
    {E_SUCCESS, "No Error"},
    {E_INVALID_INPUT__ARG_WIDTH, "The width of an argument must be at least one. This specifies how many uint64_t the arg1, arg2, ... points to."},
    {E_INVALID_INPUT__BOUNDS_NULL, "The provided bounds pointer is null."},
    {E_INVALID_INPUT__NUM_ARG_IN,  "The number of  in-arguments is invalid (must be between 1 and 6.) I.e. There must be at least one arg1 in function_check(out1[n], arg1[n])"},
    {E_INVALID_INPUT__NUM_ARG_OUT, "The number of out-arguments is invalid (must be between 1 and 6.) I.e. There must be at least one out1 in function_check(out1[n], arg1[n])"},
    {E_INVALID_INPUT__NUM_ARG_TOO_LARGE, "The function can at max take 6 parameters. I.e. Sum of number of in/out " "arguments must be <=6."},
    {E_INVALID_INPUT__NUM_BATCHES, "The given number of batches is invalid. Must be >0."},
    {E_INVALID_INPUT__BATCH_SIZE, "The given batch size is invalid. Must be >0."},
    ...
    {E_INTERNAL_RANDOMNESS__UNINITIALIZED, "Randomness was not initialized. Call init_random first."},
    ...
    {E_INTERNAL_FUNCTIONS__NOTHING_TO_UNLOAD, "Failed to unload function, because none have been loaded. "},
    {E_INTERNAL_INITIALIZE__ALLOC, "Failed to allocate memory internally. Out Of Memory?"},
